...
import { EditCanvasType } from '@/store/modules/editStore/editStore.d'
import { loadingStart, loadingFinish, loadingError } from '@/utils/plugin'
...
import {
  ...
  HistoryItemType,
  ComponentHistoryStoreType
} from './historyStore.d'

export const useHistoryStore = defineStore({
  id: 'useHistoryStore',
  state: (): ComponentHistoryStoreType => ({
    // 后退栈
    backStack: [],
    // 前进栈
    forwardStack: []
  ...
  actions: {
    ...
    createAddHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.ADD, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 更新属性记录（大小、组件属性）
    createUpdateHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.UPDATE, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 删除组件记录
    createDeleteHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.DELETE, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 移动组件记录
    createMoveHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.MOVE, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 改变层级组件记录
    createLayerHistory(
      ...
      this.createStackItem(item, type, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 剪切组件记录
    createPasteHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.CUT, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 锁定记录
    createLockHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.LOCK, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 解锁记录
    createUnLockHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.UNLOCK, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 隐藏记录
    createHideHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.HIDE, HistoryTargetTypeEnum.COMPONENT)
    },
    // * 展示记录
    createShowHistory(item: Array<CreateComponentType | CreateComponentGroupType>) {
      this.createStackItem(item, HistoryActionTypeEnum.SHOW, HistoryTargetTypeEnum.COMPONENT)
    }
  }
})
