<template>
  <div class="my-edit-select" v-if="isSelect" :style="positionStyle">
...
const { isSelect} = toRefs(editStore.getEditCanvas)
...
watch(
  ...
  positionInfo => {
    ...
    if (x > startX && y > startY) {
      ...
      attr.x = startX
      attr.y = startY
      attr.w = Math.round(x - startX)
      attr.h = Math.round(y - startY) 
    } else if (x > startX && y < startY) {
      // 右上方向
      attr.x = startX
      attr.w = Math.round(x - startX) 
      attr.h = Math.round(startY - y) 
      attr.y = startY - attr.h
    } else if (x < startX && y > startY) {
      // 左下方向
      attr.y = startY
      attr.w = Math.round(startX - x) 
      attr.h = Math.round(y - startY) 
