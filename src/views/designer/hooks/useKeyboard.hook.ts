  ...
  const switchHandle = (keyboardValue: typeof winKeyboardValue, e: string) => {
    switch (e) {
      ...
      case keyboardValue.up:
        keymaster(e, throttle(() => { editStore.setMove(MenuEnum.ARROW_UP); return false }, throttleTime))
        break;
      // ct+→
      case keyboardValue.right:
        keymaster(e, throttle(() => { editStore.setMove(MenuEnum.ARROW_RIGHT); return false }, throttleTime))
        break;
      // ct+↓
      case keyboardValue.down:
        keymaster(e, throttle(() => { editStore.setMove(MenuEnum.ARROW_DOWN); return false }, throttleTime))
        break;
      // ct+←
      case keyboardValue.left:
        keymaster(e, throttle(() => { editStore.setMove(MenuEnum.ARROW_LEFT); return false }, throttleTime))
        break;

      // 删除 delete
      // case keyboardValue.delete:
      //   keymaster(e, debounce(() => { editStore.removeComponentList(); return false }, throttleTime))
      //   break;
      // 复制 ct+v
      // case keyboardValue.copy:
      //   keymaster(e, debounce(() => { editStore.setCopy(); return false }, throttleTime))
      //   break;
      // 剪切 ct+x
      // case keyboardValue.cut:
      //   keymaster(e, debounce(() => { editStore.setCut(); return false }, throttleTime))
      //   break;
      // 粘贴 ct+v
      // case keyboardValue.parse:
      //   keymaster(e, throttle(() => { editStore.setParse(); return false }, throttleTime))
      //   break;

      // 撤回 ct+z
      // case keyboardValue.back:
      //   keymaster(e, throttle(() => { editStore.setBack(); return false }, throttleTime))
      //   break;
      // 前进 ct+sh+z
      // case keyboardValue.forward:
      //   keymaster(e, throttle(() => { editStore.setForward(); return false }, throttleTime))
      //   break;
      

      // 锁定 ct+l
      // case keyboardValue.lock:
      //   keymaster(e, throttle(() => { editStore.setLock(); return false }, throttleTime))
      //   break;
      // 解除锁定 ct+sh+l
      // case keyboardValue.unLock:
      //   keymaster(e, throttle(() => { editStore.setUnLock(); return false }, throttleTime))
      //   break;

      // 隐藏 ct+h
      // case keyboardValue.hide:
      //   keymaster(e, throttle(() => { editStore.setHide(); return false }, throttleTime))
      //   break;
      // 解除隐藏 ct+sh+h
      // case keyboardValue.show:
      //   keymaster(e, throttle(() => { editStore.setShow(); return false }, throttleTime))
      //   break;
    }
  }
