...
const getGroupStyle = (item: CreateComponentGroupType) => {
  ...
  if (path.length == 1) {
    if (item.option.Dock == GroupModeEnum.FILL) {
      return { w: editStore.getEditCanvasConfig.width, h: editStore.getEditCanvasConfig.height }
    } else if (item.option.Dock == GroupModeEnum.TOP || item.option.Dock == GroupModeEnum.BOTTOM) {
      return { w: editStore.getEditCanvasConfig.width, h: item.attr.h }
    } else if (item.option.Dock == GroupModeEnum.LEFT || item.option.Dock == GroupModeEnum.RIGHT) {
      return { w: item.attr.w, h: editStore.getEditCanvasConfig.height }
    }
    ...
    if (item.option.Dock == GroupModeEnum.FILL) {
      ...
      return { w: parentGroupStyle.w, h: item.attr.h }
    } else if (item.option.Dock == GroupModeEnum.LEFT || item.option.Dock == GroupModeEnum.RIGHT) {
      return { w: item.attr.w, h: parentGroupStyle.h }
    }
    return { w: item.attr.w, h: item.attr.h }
  }
}
...
// 容器模式
export const useDockStyle = (value: GroupModeEnum, attr: AttrType) => {
  if (!value) return {}
  if (value == GroupModeEnum.NONE) {
    return {
      width: `${attr.w}px`,
...
export const useInnerDockStyle= (value: GroupModeEnum, attr: AttrType) => {
  if (!value) return {}
  if (value == GroupModeEnum.NONE) {
    return {
      width: `${attr.w}px`,
